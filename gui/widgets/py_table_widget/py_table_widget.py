...
class PyTableWidget(QTableWidget):
    def __init__(
        ...
        self.setHorizontalHeaderLabels(['Ποσότητα', 'Περιγραφή', 'Τιμή'])
        self.horizontalHeader().setSectionResizeMode(QHeaderView.Stretch)
        self.setEditTriggers(QTableWidget.NoEditTriggers)

        # Set table selection mode and behavior
    ...
    def add_item(self, row, quantity, description, price):
        if row >= self.rowCount():
            self.setRowCount(row + 1)
        self.setItem(row, 0, QTableWidgetItem(str(quantity)))
        self.setItem(row, 1, QTableWidgetItem(description))
        self.setItem(row, 2, QTableWidgetItem(str(price)))

    def update_table(self, data):
        self.setRowCount(len(data))
        for row, (quantity, description, price) in enumerate(data):
            self.setItem(row, 0, QTableWidgetItem(str(quantity)))
            self.setItem(row, 1, QTableWidgetItem(description))
            self.setItem(row, 2, QTableWidgetItem(str(price)))
